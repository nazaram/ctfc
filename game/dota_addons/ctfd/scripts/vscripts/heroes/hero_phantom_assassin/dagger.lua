...
function daggerthrow( keys )
	...
	local particle_name 	= "particles/units/heroes/hero_phantom_assassin/phantom_assassin_stifling_dagger.vpcf"
	local sound_cast		= "Hero_PhantomAssassin.Dagger.Cast"
	local sound_impact		= "Hero_PhantomAssassin.Dagger.Target"

	local max_distance		= ability:GetLevelSpecialValueFor("AbilityCastRange", ability_level)
	local vision_block		= ability:GetLevelSpecialValueFor("vision_block", ability_level)
